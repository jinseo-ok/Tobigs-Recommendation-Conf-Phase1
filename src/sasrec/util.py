...
def evaluate(model, emb, dataset, max_len):
    ...
    for u in users:
        ...
        for _ in range(100):
            ...
            while t in rated: # 구매했던 item을 제외하고 100개를 뽑는 과정
                t = np.random.randint(1, itemnum + 1)
            item_idx.append(t)

        test_emb = emb(np.pad(np.array(item_idx).reshape(-1, 1),
                       ((0, 0), (max_len-1, 0)), 'constant'))[:, -1] # 구매하지 않은 item에 대한 embedding
...
def evaluate_valid(model, emb, dataset, max_len):
    ...
    for u in users:
        ...
        valid_emb = emb(np.pad(np.array(item_idx).reshape(-1, 1),
                        ((0, 0), (max_len-1, 0)), 'constant'))[:, -1] # 구매하지 않은 item에 대한 embedding
        seq_emb = (-model.predict(seq.reshape(1, max_len)))[:, -1] # 구매한 item에 대한 model의 user 추정값
        predictions = tf.matmul(seq_emb, valid_emb, transpose_b=True) 
        predictions = predictions[0].numpy()

        rank = predictions.argsort().argsort()[0] # 첫번째 item의 relevance

        valid_user += 1 # user 수 

        if rank < 10: # 첫번째 item의 relevance가 10 위 미만일 때, -> 왜 이 조건을 사용했을까?
            NDCG += 1 / np.log2(rank + 2)
            HT += 1
        if valid_user % 100 == 0:
            print('Validated {:.0f} users\r'.format(valid_user), end='')
